...
%
    ...
    celldata = (struct2cell(data));     % Convert struct to cell array (column vector shaped with nested cell arrays (=data fields) as entrys)
    celldata = horzcat(celldata{:});        % Unfold nested cell arrays to single cell array (matrix shaped: each column represents a data field)
    filepath = fullfile(filepath, filename + ".csv");           % Get full filepath of .csv file
    writecell(celldata,filepath, "Delimiter", ",");                 % Write to .csv file
