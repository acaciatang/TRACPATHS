function R = locate16BitCodes(im, varargin)
    ...
    images = dir('5040/*.png');
    tags = readtable('TagList.csv', 'NumHeaderLines',1);
    for iter = 1:length(images)
        im = imread(['5040/' images(iter).name]);
        for i = 1:height(tags)
           if contains(images(iter).name,tags{i,1})
               validTagList = tags(i,:);
               validTagList(:,1) = [];
               validTagList = table2array(validTagList);
               validTagList = validTagList(2:end);
           end
        end    
    ...
    for i = 1:numel(R)
        ...
        try
            ...
            [isq,cnr] = fittoquad( R(i).BoundingBox, R(i).FilledImage);
    ...
    if vis==1
        ...
        saveas(gcf,['Results/' images(iter).name '_Crall.png'])
    end
    ...
    for j = 1:length(R)
        ...
        trackingDataReshaped(j).centroidX = centroid(1);
        trackingDataReshaped(j).centroidY = centroid(2);
        trackingDataReshaped(j).frontX = row.frontX;
        trackingDataReshaped(j).frontY = row.frontY;
        trackingDataReshaped(j).ID = row.number
    end
       
    writestruct(trackingDataReshaped, strcat('Results/', images(iter).name, 'trackingData.json'))
