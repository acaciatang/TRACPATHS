...
LISTs = readtable('TagList.csv','NumHeaderLines',1);
IDs = table2array(LISTs(:,1));
ID = filename(1:7);

INDEX = find(strcmp(IDs, filename(1:7)));

codelist = table2array(LISTs(INDEX, 2:end));
...
for i = 1:nframes
    ...
    for j = 1:numel(codelist)
        ...
        if ~isempty(F)
            ...
            if ~isempty(FS)
                hold = FS.Centroid;
                trackingDataReshaped(j).CentroidX(i) = hold(1);
                trackingDataReshaped(j).CentroidY(i) = hold(2);
...
%% Save data
time = cputime-start;
save(strcat(filename, num2str(time), 'trackingData.mat'), 'trackingDataReshaped')

%% Replay video
disp('replaying video with tracking data shown');
TD = trackingDataReshaped;

outputMovieName = strcat(filename, num2str(time), '.avi');
